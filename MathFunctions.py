...
def sin(x, precision = 5):
    ...
    for n in range(1, precision):
        print "-1^", n, ":", (-1.0) ** n 
        print "!", 2 * n + 1, " = (2 * ", n, " + 1): ", factorial(2 * n + 1)
        print x, "^(2", n, "+1): ", x ** (2 * n + 1)
        answer += ((-1.0) ** n) / factorial((2 * n + 1)) * (x ** (2 * n + 1))
        print "answer:", answer
        ...
        if isOp(expression[count]):
            ...
            if expression[count] == '-' and (count - 1 < 0 or isOp(expression[count - 1]) or expression[count - 1] == '('):
                ...
                if len(operatorStack) >= 1:
                    if opPrec == getPrecedence(operatorStack[len(operatorStack) - 1]):
                        if expression[count] != '^':
                            outputStr += operatorStack[len(operatorStack) - 1] + " "    
